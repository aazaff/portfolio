...
if (suppressWarnings(require("vegan"))==FALSE) {
    install.packages("vegan",repos="https://cran.microsoft.com/");
    library("vegan");
    }
...
# External packages are almost always explicitly invoked per function with :: operator
# Explict package calls are not required in most cases, but are helpful for readers
...
HamiltonDCA = vegan::decorana(HamiltonCull)
HamiltonScores = vegan::scores(HamiltonDCA,display="sites")
