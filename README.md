...
My specialty in ecology is multivariate anlaysis (e.g., ordination, cluster analysis, principal components analysis). These methods take contingency tables of samples with multiple characteristics (i.e., multiple variables), and group sites based on the similarity the observed characteristics. For example, we could measure mutliple heavy metal concentrations (e.g., cesium, thorium) in various water bodies, then agnostically extract similar types of pollution among the different water bodies. This is a powerful tool for both basic data exploration and hypothesis testing.
...
> Ancient sediments deposited in New York during this time (380 ma) follow an unsual geographid distribution. Sediments deposited in deep water are bracketed to the east and west by sediments from shallower-water. To the west was a carbonate bank (tropical reef like environment) and to the east was a siliciclastic shelf (sandy beach type environment). This west to east separation of samples is visible in the DCA of Hamilton samples (A). Samples from western New York counties (reef; blue) are separated in ordination space from samples from the eastern (sandy; red) part of the state (A,B).
